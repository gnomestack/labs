...
public class Os_Tests
{
    ...
    public void Current_DoesNotThrow(ITestOutputHelper output)
    {
        ...
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            ...
            Assert.False(Os.IsWasi());
            Assert.False(Os.IsMacOS());
            Assert.False(Os.IsLinux());
        }
        ...
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
        {
            ...
            Assert.False(Os.IsMacOS());
            Assert.False(Os.IsWindows());
            Assert.False(Os.IsBrowser());

            if (current.Id.Equals("ubuntu"))
            {
                Assert.True(Os.IsUbuntu());
            }
            ...
            if (File.Exists("/etc/os-release"))
            {
                ...
                foreach (var line in lines)
                {
                    ...
                    if (line.StartsWith("VERSION="))
                    {
                        ...
                        Assert.Equal(version, current.VersionLabel);
                    }
